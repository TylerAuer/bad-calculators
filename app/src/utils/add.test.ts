...
const one = genOp({ symbol: OpType.add, value: 1 });
const negOne = genOp({ symbol: OpType.add, value: -1 });
const onePtTwo = genOp({ symbol: OpType.add, value: 1.2 });
...
describe('fixed operand is a float', () => {
  ...
  test('+ float', () => {
    expect(onePtTwo.op(1.1)).toBe(2.3);
  });
  ...
  test('+ negative float', () => {
    expect(onePtTwo.op(-1.5)).toBe(-0.3);
  });
});
